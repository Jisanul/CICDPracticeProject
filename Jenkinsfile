...
def dotnet_build(){
		sh(script: 'dotnet build CICDPracticeProject.csproj', returnStdout: true);
}
...
def dotnet_publish(){
        sh(script: 'dotnet publish CICDPracticeProject.csproj -o ./obj/Docker/publish', returnStdout: true);
...
def createContainer(){
    ...
    def payload = sh(\'{ "Image": "netcoreapp:' + VERSION_NUMBER + '", "ExposedPorts": { "5000/tcp" : {} }, "HostConfig": { "PortBindings": { "5000/tcp": [{ "HostPort": "5000" }] } } }\');
    def createResponse = dockerApiRequest('containers/create', 'POST', 'json', 'json', payload);
